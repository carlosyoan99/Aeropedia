...
function buildSummary() {
  ...
  const gen5Count      = fleetsDB.reduce((s, c) => {
    return s + c.aircraft.reduce((ss, a) => {
      const p = aircraftDB.find(p => p.id === a.id);
      return ss + (p && p.generation === '5' && a.qty > 0 ? a.qty : 0);
    }, 0);
  }, 0);
  const f35Total = fleetsDB.reduce((s, c) => 
    s + c.aircraft.reduce((ss, a) => ss + (a.id === 'f35' ? a.qty : 0), 0)
  , 0);
...
function renderFleets() {
  ...
  let data = fleetsDB.filter(c => {
    const matchSearch = !search || c.country.toLowerCase().includes(search) || c.aircraft.some(a => a.name.toLowerCase().includes(search));
...
function buildFleetCard(country) {
  // Quick stats
  const totalAircraft = country.aircraft.reduce((s, a) => s + (a.qty || 0), 0);
  ...
  const gen5Count = country.aircraft.reduce((s, a) => {
    const p = aircraftDB.find(p => p.id === a.id);
    return s + (p && p.generation === '5' ? (a.qty || 0) : 0);
  }, 0);
  ...
  const rows = country.aircraft.map(a => {
    ...
    return `<tr onclick="viewPlaneFromFleet(\'${a.id}\')" title="Ver ficha de ${a.name}">
      <td style="width:50px"><img src="public\/min\/${a.img}.webp" class="fat-img" onerror="this.src=\'${FALLBACK_IMG}\'"></td>
      ...
      <td td class="fat-note hidden-sm" >${a.notes}</td>
      </tr>`
  }).join('');
  ...
  const typeBarSegs = Object.entries(typeGroups).map(entry => {
